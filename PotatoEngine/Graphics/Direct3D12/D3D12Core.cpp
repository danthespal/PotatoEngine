...
// TODO: remove when done how to create a root signature the tendious way
...
private:
	struct command_frame
	{
		...
		void wait(HANDLE fence_event, ID3D12Fence1* fence)
		{
			...
			{
				// i have the fence create an event wich is singaled one the fence's current value equals "fence_value"
...
//		 output devices (ex. screens) are attached, enumerate the supported resolutions, provide
...
process_deferred_releases(u32 frame_idx)
{
	...
	// NOTE: clear this flag in the beginning. if we'd clear it at the end
...
initialize()
{
...
#ifdef _DEBUG
	// enable debbuging layer. requires "Graphics Tools" optional feature
	{
	...
	// determine wich adapter (e.x. graphics card) to use, if any
...
shutdown()
{
	...
	// NOTE: don't call process_deferred_releases at the end because
	...
	//		 process_deferred_releases one more
...
void create_a_root_signature()
{
	...
	// only one resource heap and one sampler can be set at any time
